    ...
    public partial class DatabaseContext : DbContext
    {
        ...
        protected void Upgrade()
        {
            using (IDbConnection connection = new SqlConnection(ConnectionString))
            {
                ...
                var oldHistoryExists = connection.TableCount("__MigrationHistory") > 0;
                if (!oldHistoryExists) return;

                var lastOldMigration = connection
                    .SqlQuery<string>(
                ...
                var newHistoryExists = connection.TableCount("__EFMigrationsHistory") > 0;
                if (!newHistoryExists)
                ...
                if (!newHistoryExists)
                    throw new Exception(
                        "Could not create new Migration History record in Table '__EFMigrationsHistory'");
            }
        }
    }
}