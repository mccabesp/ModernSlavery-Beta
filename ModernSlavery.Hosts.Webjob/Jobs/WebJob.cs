...
namespace ModernSlavery.Hosts.Webjob.Jobs
{
    ...
    public class WebJob : IWebJob, IDisposable
    {
        #region Properties
        ...
        public class OncePerNightRandomSchedule : DailySchedule
        {
            public override DateTime GetNextOccurrence(DateTime now)
            {
                var _closeOfBusiness = new DateTime(now.Year, now.Month, now.Day, 19, 0, 0);
                if (now > _closeOfBusiness) _closeOfBusiness=_closeOfBusiness.AddDays(1);

                _closeOfBusiness=_closeOfBusiness.AddHours(Numeric.Rand(0, 12));
                _closeOfBusiness=_closeOfBusiness.AddMinutes(Numeric.Rand(0, 59));
                _closeOfBusiness=_closeOfBusiness.AddSeconds(Numeric.Rand(0, 59));
                return _closeOfBusiness;
            }
        }
