...
{
    public class FixOrganisationAddressesWebJob : WebJob
    {
        #region Dependencies
        ...
        public FixOrganisationAddressesWebJob(
        ...
        public async Task FixOrganisationAddressesAsync([TimerTrigger(typeof(OncePerWeekendRandomSchedule))]
            TimerInfo timer,
                ILogger log)
        {
            if (RunningJobs.Contains(nameof(FixOrganisationAddressesAsync))) return;
            RunningJobs.Add(nameof(FixOrganisationAddressesAsync));
            try
            {
                ...
                log.LogDebug($"Executed WebJob {nameof(FixOrganisationAddressesAsync)} successfully");
            }
            catch (Exception ex)
            {
                var message = $"Failed webjob ({nameof(FixOrganisationAddressesAsync)}):{ex.Message}:{ex.GetDetailsText()}";
                log.LogError(ex, $"Failed webjob ({nameof(FixOrganisationAddressesAsync)})");
            ...
            {
                RunningJobs.Remove(nameof(FixOrganisationAddressesAsync));
            }
