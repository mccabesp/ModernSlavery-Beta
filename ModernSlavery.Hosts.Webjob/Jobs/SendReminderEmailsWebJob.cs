...
{
    public class SendReminderEmailsWebJob : WebJob
    {
        #region Dependencies
        ...
        public SendReminderEmailsWebJob(
        ...
        private async Task SendReminderEmailAsync(User user, SectorTypes sectorType, List<Organisation> organisations)
        {
            var personalisation = new Dictionary<string, dynamic>
            {
                {
                    "DeadlineDate",
                    _reportingDeadlineHelper.GetReportingDeadline(sectorType).ToString("d MMMM yyyy")
                },
                {
                    "DaysUntilDeadline",
                    _reportingDeadlineHelper.GetReportingDeadline(sectorType).Subtract(VirtualDateTime.Now).Days
                },
        ...
        {
            return _reportingDeadlineHelper.GetReportingDeadline(sectorType);
        }
    }
}