...
namespace ModernSlavery.Hosts.Web.Tests
{
    [TestFixture]
    [Parallelizable]
    public class WebHostSanityTest: BaseUITest
    {
        public WebHostSanityTest():base(TestRunSetup.TestWebHost,TestRunSetup.WebDriverService)
        {
        ...
        public async Task WebTestHost_HttpGet_ReturnsIdentityDiscovery()
        {
            ...
            var response = await WebRequestAsync(HttpMethods.Get, discoveryAuthority, validateCertificate: false).ConfigureAwait(false);
        ...
        public async Task WebTestHost_HttpGet_ReturnsValidResponse()
        {
            ...
            //Check we get a response from the test web server
            var response =await WebRequestAsync(HttpMethods.Get,_webAuthority, validateCertificate:false).ConfigureAwait(false);
            Assert.That(!string.IsNullOrWhiteSpace(response));
        }
        ...
        public async Task WebTestHost_SeleniumHelper_TestMethods_OK()
        {
            ...
            await this.CheckAccessibilityAsync().ConfigureAwait(false);

            //Check the accessibility of the current page saving results to {UrlPath}_POST.html
            await this.CheckAccessibilityAsync(httpMethod:"POST").ConfigureAwait(false);
            ...
            await this.CheckAccessibilityAsync("manage-organisations").ConfigureAwait(false);
            ...
            await this.CheckAccessibilityAsync($"/statement/group-report/{TestContext.CurrentContext.Test.Name}").ConfigureAwait(false);
