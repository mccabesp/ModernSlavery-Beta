...
namespace ModernSlavery.WebUI.Tests.Controllers.Admin
{
    ...
    public class AdminControllerManualChangesSecurityCodeTests
    {
        ...
        private static Core.Entities.Organisation SetOrganisation(Mock<Core.Entities.Organisation> org, int i, OrganisationStatuses orgStatus)
        {
            Core.Entities.Organisation organisation = org.Object;
        ...
        {
            var listOfOrganisations = new List<Core.Entities.Organisation>();
            for (var i = 0; i < 1000; i++)
            {
                Core.Entities.Organisation organisation;
                var org = new Mock<Core.Entities.Organisation> {CallBase = true};
        ...
        public async Task AdminController_ManualChanges_POST_Expire_Security_Code_In_Bulk_Succeeds_When_Run_In_Test_ModeAsync()
        {
            ...
            mockedDatabase.SetupGetAll((ICollection<Core.Entities.Organisation>) listOfOrganisations);
            mockedDatabase.Setup(x => x.Get<User>(It.IsAny<long>())).Returns(_databaseAdminUser);
        ...
        public async Task AdminController_ManualChanges_POST_Create_Security_Code_In_Bulk_Succeeds_When_Run_In_Test_ModeAsync()
        {
            ...
            {
                Core.Entities.Organisation organisation;
                var org = new Mock<Core.Entities.Organisation>();
