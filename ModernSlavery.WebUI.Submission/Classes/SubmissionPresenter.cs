...
namespace ModernSlavery.WebUI.Submission.Classes
{
    ...
    public class SubmissionPresenter : ISubmissionPresenter
    {
        ...
        {
            return await SubmissionService.SharedBusinessLogic.DataRepository.FirstOrDefaultAsync<Return>(r => r.ReturnId == returnId);
        }
        ...
        public async Task<Return> GetReturnFromDatabaseAsync(long organisationId, int snapshotYear)
        {
            ...
            return await SubmissionService.SharedBusinessLogic.DataRepository.FirstOrDefaultByDescendingAsync<Return,long>(s => s.ReturnId,
                    s => s.Status == ReturnStatuses.Submitted && s.AccountingDate.Year == snapshotYear && s.OrganisationId == organisationId);
        }
        ...
        public async Task<ReturnViewModel> GetReturnViewModelAsync(long organisationId, int snapshotYear, long userId)
        {
            ...
            UserOrganisation userOrganisationFromDatabase = await SubmissionService.SharedBusinessLogic.DataRepository.FirstOrDefaultAsync<UserOrganisation>(uo => uo.UserId == userId && uo.OrganisationId == organisationId);
        ...
        private async Task<ReportInfoModel> GetReport_InfoAsync(Organisation organisation, DateTime? returnModifiedDate, int snapshotYear)
        {
            ...
            var result = new ReportInfoModel {
                ...
                ReportingRequirement =
                    await SubmissionService.ScopeBusinessLogic.GetLatestScopeStatusForSnapshotYearAsync(organisation.OrganisationId, snapshotDate.Year)
            };
        ...
        {
            return await SubmissionService.DraftFileBusinessLogic.GetDraftIfAvailableAsync(organisationId, snapshotYear);
        }

        public async Task UpdateDraftFileAsync(long userRequestingTheUpdate, ReturnViewModel returnViewModel)
        {
            await SubmissionService.DraftFileBusinessLogic.UpdateAsync(returnViewModel, returnViewModel.ReportInfo.Draft, userRequestingTheUpdate);
        }

        public async Task<Draft> GetDraftFileAsync(long organisationId, int snapshotYear, long userIdRequestingDraft)
        {
            return await SubmissionService.DraftFileBusinessLogic.GetExistingOrNewAsync(organisationId, snapshotYear, userIdRequestingDraft);
        }

        public async Task KeepDraftFileLockedToUserAsync(ReturnViewModel returnViewModel, long userIdRequestingLock)
        {
            await SubmissionService.DraftFileBusinessLogic.KeepDraftFileLockedToUserAsync(returnViewModel.ReportInfo.Draft, userIdRequestingLock);
        }
        ...
        {
            await SubmissionService.DraftFileBusinessLogic.RestartDraftAsync(organisationId, snapshotYear, userIdRequestingDraft);
        }
