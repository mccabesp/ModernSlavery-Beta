...
namespace ModernSlavery.WebUI.Shared.Classes.Middleware
{
    public class BasicAuthenticationMiddleware
    {
        ...
        private static string _Realm; // HTTP.1.1 Basic Challenge Realm 

        private readonly RequestDelegate _next;
        private readonly string _Password;
        private readonly string _Username;

        public BasicAuthenticationMiddleware(RequestDelegate next, string username = null, string password = null,
            string realm = null)
        {
        ...
        protected virtual (string Username, string Password) ExtractBasicCredentials(string authorizationHeader)
        {
            ...
            if (verifiedAuthorizationHeader.IndexOf(HttpBasicSchemeName) != 0)
                throw new ArgumentException($"Authentication scheme is not '{HttpBasicSchemeName}'",
            ...
            if (separatorPosition < 0)
                throw new ArgumentException($"Missing credential separator '{HttpCredentialSeparator}'",
                    nameof(authorizationHeader));

            var username = decodedAuthorizationHeader.Substring(0, separatorPosition).Trim();
            var password = decodedAuthorizationHeader
        ...
        protected virtual void IssueAuthenticationChallenge(HttpContext httpContext)
        {
            ...
            httpContext.SetResponseHeader(HttpWWWAuthenticateHeader,
                $"Basic {(string.IsNullOrWhiteSpace(_Realm) ? "" : $"\"{_Realm}\"")}");
        }
    }
}