...
namespace ModernSlavery.WebUI.Shared.Classes.HttpResultModels
{
    ...
    public class HttpStatusViewResult : ViewResult
    {
        ...
        public HttpStatusViewResult(HttpStatusCode statusCode, string statusDescription = null,
            LogLevel? logLevel = null) : this(
            (int) statusCode,
            statusDescription,
            logLevel)
        {
        ...
        public override Task ExecuteResultAsync(ActionContext actionContext)
        {
            ...
            if (logLevel == null)
            {
                ...
                else if (StatusCode >= 400) logLevel = LogLevel.Error;
            }

            if (logLevel != null && logLevel != LogLevel.None)
            {
                var log = actionContext.HttpContext.RequestServices
                    ?.GetRequiredService<ILogger<HttpStatusViewResult>>();
            ...
            var errorViewModelFactory =
                actionContext.HttpContext.RequestServices?.GetRequiredService<IErrorViewModelFactory>();

            ViewName = "Error";

            ViewData = new ViewDataDictionary(new EmptyModelMetadataProvider(), actionContext.ModelState)
            {
                Model = errorViewModelFactory.Create((int) StatusCode) // set the model
            };
